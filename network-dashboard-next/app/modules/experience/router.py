...
def overview(
    ...
    experience: ExperienceService = Depends(service),
) -> dict:
...
def compass(request: Request, identity: Identity = Depends(require_login)) -> dict:
    forecast = request.app.state.weather.forecast() if identity.admin or "weather" in identity.sections else {}
    ...
    if (
        (identity.admin or "family" in identity.sections)
        and secret
        and request.app.state.database.table_exists("presence_devices")
    ):
        try:
...
def create_forget_item(
    payload: ForgetItemCreate,
    identity: Identity = Depends(require_login),
    experience: ExperienceService = Depends(service),
) -> dict:
    return {"ok": True, "id": experience.add_forget_item(payload.model_dump(), identity)}


@router.patch("/forget-items/{item_id}", dependencies=[Depends(require_same_origin)])
def update_forget_item(
    item_id: str,
    payload: ForgetItemUpdate,
    identity: Identity = Depends(require_login),
    experience: ExperienceService = Depends(service),
) -> dict:
    try:
        experience.update_forget_item(
            item_id,
...
def delete_forget_item(
    item_id: str,
    identity: Identity = Depends(require_login),
    experience: ExperienceService = Depends(service),
) -> dict:
    try:
        experience.delete_forget_item(item_id, identity)
        return {"ok": True}
    except PermissionError as exc:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail=str(exc)) from exc
    except ValueError as exc:
