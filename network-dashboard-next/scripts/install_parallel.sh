...
LIVE_ORIGIN="${LEGACY_ORIGIN:-http://127.0.0.1:8792}"
...
chmod 600 "$HOME/.cache/network-dashboard-next/last-install.env"
...
"$VENV/bin/python" "$APP_DIR/scripts/readiness_report.py" \
  ...
  --google-client "$GOOGLE_CLIENT_SECRETS_PATH" \
  --legacy-report "$LEGACY_IMPORT_REPORT" \
  --service network-dashboard-next.service > "$READINESS_REPORT"
READINESS_EXIT=$?
set -e
...
if [[ "$READINESS_EXIT" -ne 0 ]]; then
  echo "Readiness-kontrollen blockerade installationen. Next-tjänsten stoppas; liveappen påverkas inte." >&2
  systemctl --user stop network-dashboard-next.service || true
  systemctl --user status network-dashboard-next.service --no-pager || true
  exit "$READINESS_EXIT"
fi
...
Readiness-rapport: $READINESS_REPORT
...
Gamla appen på $LIVE_ORIGIN har inte ändrats.
EOF
...
Readiness igen:
  $VENV/bin/python $APP_DIR/scripts/readiness_report.py --origin "$ORIGIN"
Jämför API-kontrakt:
